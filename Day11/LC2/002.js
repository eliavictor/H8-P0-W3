...
function activityNotifications(expenditure, d) {
	...
	if (indexMid%2 == 0) {
		indexMid = expenditure.indexLength/2;
		median = expenditure[indexMid]
	} else {
		indexMid = expenditure.(indexLength+1)/2;
		median = expenditure[indexMid]
	}

	for (d; d <= expenditure.length; d++) {
		if (expenditure[d+1] >= expenditure[i] * 2) {
    	counter++
    	}
	}
